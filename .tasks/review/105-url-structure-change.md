...
- [x] `app/(app)/admin/[siteId]/` 내용을 `app/(app)/admin/`으로 이동
- [x] layout.tsx 수정 (URL에서 siteId 추출 제거)
- [x] `[siteId]` 디렉토리 삭제

### Phase 2: 페이지 업데이트

- [x] 모든 페이지에서 `useParams` → `useSiteId()` 변경
- [x] 대시보드 페이지 `/admin/dashboard`로 이동

### Phase 3: 링크 업데이트

- [x] `AdminSidebar.tsx` 링크 수정
- [x] `SiteSwitcher.tsx` 네비게이션 수정
- [x] 페이지 내 내부 링크 수정

### Phase 4: 리다이렉트 설정

- [x] `next.config.js`에 기존 URL 리다이렉트 추가
- [x] `/admin/[siteId]/*` → `/admin/*` 리다이렉트

### Phase 5: 테스트

- [x] 모든 Admin 페이지 접근 확인
- [x] 사이트 전환 동작 확인
- [x] 기존 URL 리다이렉트 확인
